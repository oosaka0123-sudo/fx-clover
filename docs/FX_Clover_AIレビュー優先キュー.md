...
- `p0_master_course_blog_review.json`
- `p0_dma_ma_timeframe_blog_review.json`
...
初回P0先頭は公式マスター講座のうち、右肩・環境認識・エントリー条件に直接近い記事から並んだ。

## P0マスター講座7件レビュー後

Issue #18で次の公式ブログ本文を一次資料レビューした。
...
この7件は `knowledge/p0_master_course_blog_review.json` へ移り、キューではDONEになった。

次のP0先頭は:

1. `8268` — ずらした移動平均線DMA / エントリーと決済
2. `7691` — 上位MAが下位を支配する
3. `6979` — 時間足を変えたライン解釈
4. `7022` — トレンドライン3-3クロス
5. `7891` — 200/600MAパターン
6. `8090` — 1時間MAとレート位置でロジック選択

となり、未解決のDMA・上位環境・3-3・MA障害へ自然に研究順が移った。

## Issue #20 P0 DMA / MA / 時間足6件レビュー後

Issue #20で次の公式ブログ本文を一次資料レビューした。

- 8268 — ずらした移動平均線DMA / エントリーと決済
- 7691 — 上位MAが下位を支配する
- 6979 — 時間足を変えたライン解釈
- 7022 — トレンドライン3-3クロス
- 7891 — 200/600MAパターン
- 8090 — 1時間MAとレート位置でロジック選択

レビュー後の状態:

- `EVIDENCE_REVIEWED`: 27
- `UNREVIEWED`: 144
...
この6件は `knowledge/p0_dma_ma_timeframe_blog_review.json` へ移り、キューではDONEになった。

今回の一次資料では、上位/下位時間足の連携、MAとレートの位置関係による手法選択、MA障害と伸びしろ、右肩を含む根拠重合、DMAのエントリー/決済利用が補強された。一方、普遍的な新規数値閾値、固定点数制、万能な200/600条件、3-3必須化を正当化する根拠は得られていない。

現在のP0は `8343 / 7913 / 7936 / 8108 / 8520 / 8984 / 9033 / 9131`。研究順であり、売買ルールの重要度を意味しない。
